	...
	protected NodeInfo getPred(NodeInfo info) throws Failed {
		...
		if (localInfo.addr.equals(info.addr)) {
			...
			if (doc.getElementsByTagName("id").getLength() > 0) {
				String str_id = doc.getElementsByTagName("id").item(0).getTextContent();
				int id = Integer.parseInt(str_id);
				String addr = doc.getElementsByTagName("addr").item(0).getTextContent();
				return new NodeInfo(id, new URI(addr));
			} else {
				return null;
			}
		}
...
//		routing.joinCheck();
	...
	protected NodeInfo closestPrecedingFinger(NodeInfo info, int id) throws Failed {
		...
		if (localInfo.equals(info)) {
			...
			if (IRouting.USE_FINGER_TABLE) {
				...
				{
					Document doc;
					doc = getRequest(String.format(info.addr + UriApi.GET_NODE_BY_FINGER, i));
					int finger_id = Integer.parseInt(doc.getElementsByTagName("id").item(0).getTextContent());
					String finger_addr = doc.getElementsByTagName("addr").item(0).getTextContent();
					if(inInterval(finger_id, info.id, id, false))
					{
						return new NodeInfo(finger_id, new URI(finger_addr));
					}
				}
	...
	protected NodeInfo findPredecessor(int id) throws Failed {
		...
		if (info.id != succ.id) {
			while (!inInterval(id, info.id, succ.id)) {
				...
				succ = getSucc(info);
				/*
				 * Break out of infinite loop (e.g. our successor may be a
				 * single-node network that still points to itself).
				 */
	...
	public boolean stabilize() throws Failed {
		...
		if (predOfSucc == null) {
			 ...
			 * 
			 * Note: We pass in our bindings to the successor for backing up. We
			 * don't use this now, but might in a future assignment.
			 * 
			...
			/*
			 * Note: We notify predOfSucc in this case, since we have set that
			 * as our successor.
			 */
			TableRep db = client.notify(predOfSucc, state.extractBindings());
			debug("Ending stabilize() 3");
			// The bindings we got back will be merged with our current
			// bindings.
			return notifyContinue(db);
	...
	protected String[] get(NodeInfo n, String k) throws Failed {
		if (n.addr.equals(info.addr)) {
			...
			try {
				...
				String[] strs = new String[list.getLength()];
				for (int i=0; i< list.getLength(); i++) {
					strs[i] = list.item(i).getTextContent();
				}
				return strs;
			} catch (Exception e) {
	...
	public Document getRequest(String uri) throws Exception {	
		URI u = new URI(uri);
		DocumentBuilderFactory factory = DocumentBuilderFactory.newInstance();
		DocumentBuilder builder = factory.newDocumentBuilder();
		Response r = client.getRequest(u);
	...
	public void putRequest(String uri) {
		try {
			URI u = new URI(uri);
			client.putRequest(u, Entity.text(""));			
		} catch (Exception e) {
			e.printStackTrace();
		}
	}
	...
	public void join(String uri) throws Failed, Invalid {
		...
		try {
			...
			int succ_id = Integer.parseInt(doc.getElementsByTagName("id").item(0).getTextContent());
			String succ_addr = doc.getElementsByTagName("addr").item(0).getTextContent();
			succ = new NodeInfo(succ_id, new URI(succ_addr));
			setSucc(succ);
			stabilize();
		} catch (Exception e) {
			e.printStackTrace();
